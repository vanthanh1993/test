...
def parse_money(val):
    return int((val or "0").replace('.', '').replace(',', ''))

def safe_sum(row):
    return row['sum'] if row and row['sum'] else 0
...
# ===== INIT DB =====
def init_db():
    execute("CREATE TABLE IF NOT EXISTS users(id SERIAL PRIMARY KEY, username TEXT, password TEXT)")
    execute("CREATE TABLE IF NOT EXISTS suppliers(id SERIAL PRIMARY KEY, name TEXT, phone TEXT, address TEXT)")
    execute("CREATE TABLE IF NOT EXISTS supplier_payments(id SERIAL PRIMARY KEY, supplier_id INT, amount INT, date TEXT)")

    execute("""CREATE TABLE IF NOT EXISTS import_logs(
        id SERIAL PRIMARY KEY, name TEXT, price INT, qty INT, total INT,
        date TEXT, supplier_id INT, paid INT DEFAULT 0, debt INT DEFAULT 0)""")

    execute("""CREATE TABLE IF NOT EXISTS imei(
        imei TEXT PRIMARY KEY, product_name TEXT, import_price INT,
        sell_price INT, status TEXT, import_date TEXT,
        sell_date TEXT, customer_id INT, import_id INT)""")

    execute("CREATE TABLE IF NOT EXISTS customers(id SERIAL PRIMARY KEY, name TEXT, phone TEXT, address TEXT)")

    execute("""CREATE TABLE IF NOT EXISTS sales_orders(
        id SERIAL PRIMARY KEY, date TEXT, customer_id INT,
        total INT, paid INT, debt INT)""")

    execute("""CREATE TABLE IF NOT EXISTS sales_items(
        id SERIAL PRIMARY KEY, order_id INT, imei TEXT,
        price INT, import_price INT)""")

    execute("""CREATE TABLE IF NOT EXISTS debt_payments(
        id SERIAL PRIMARY KEY, customer_id INT, amount INT, date TEXT)""")
...
def dashboard():
    ...
    revenue = query_one("SELECT SUM(total) FROM sales_orders")
    profit = query_one("SELECT SUM(price - import_price) FROM sales_items")
    debt = query_one("SELECT SUM(debt) FROM import_logs")
...
def import_goods():
    if request.method == 'POST':
        ...
        for im in imeis:
            try:
                c.execute("""
                    INSERT INTO imei VALUES (%s,%s,%s,0,'in_stock',%s,'',NULL,%s)
        ...
        total = success * price
        c.execute("UPDATE import_logs SET qty=%s,total=%s,debt=%s WHERE id=%s",
...
def delete_import(id):
    if query_one("SELECT 1 FROM imei WHERE import_id=%s AND status!='in_stock'", (id,)):
...
def edit_import(id):
    ...
    if request.method == 'POST':
        ...
        c.execute("UPDATE imei SET product_name=%s, import_price=%s WHERE import_id=%s",
                  (name, price, id))
...
def supplier_detail(id):
    ...
    payments = query_all("SELECT * FROM supplier_payments WHERE supplier_id=%s", (id,))

    total_import = sum(i['total'] or 0 for i in imports)
...
def delete_supplier(id):
    if query_one("SELECT 1 FROM import_logs WHERE supplier_id=%s", (id,)) \
    or query_one("SELECT 1 FROM supplier_payments WHERE supplier_id=%s", (id,)):
...
def supplier_pay():
    ...
    c.execute("""
        ...
        VALUES (%s,%s,%s)
    """, (supplier_id, amount, now_vn()))

    # Trừ nợ đúng từng dòng (QUAN TRỌNG)
    for r in rows:
        if amount <= 0:
            break
